...
{
    public partial class hamburguesas : Form
    {
        private Form home;
        ...
        public hamburguesas(Form home)
        {
            InitializeComponent();
            this.home = home;
        }

        private void hamburguesas_Load(object sender, EventArgs e)
        {
            for (int i = 0; i <= 5; i++)
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if ((int)comboBox1.SelectedItem != 0)
            {
                ...
                pedidoController.agregarItemMenu(comidas[0], num1);
            }

            if ((int)comboBox2.SelectedItem != 0)
            {
                int num2 = (int)comboBox2.SelectedItem;
                pedidoController.agregarItemMenu(comidas[1], num2);
            }

            if ((int)comboBox3.SelectedItem != 0)
            {
                int num3 = (int)comboBox3.SelectedItem;
                pedidoController.agregarItemMenu(comidas[2], num3);
            }
